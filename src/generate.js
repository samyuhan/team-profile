...
generate = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (role === 'Manager') {
            const manager = managerScript(employee);
            teamCards.push(manager);
        }
        if (role === 'Engineer') {
            const engineer = engineerScript(employee);
            teamCards.push(engineer);
        }
        if (role === 'Intern') {
            const intern = internScript(employee);
            teamCards.push(intern);
        }
        
    }

    const employees = teamCards.join('')
    const team = pageScript(employees); 
    return team;

}

const pageScript = function (employees) {   
  return`
  <!DOCTYPE html>
  <html lang="en">
  <head>
